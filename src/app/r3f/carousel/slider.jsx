import { useSliderR3f } from "@/app/store/useSliderR3f";
import { useGSAP } from "@gsap/react";
import gsap from "gsap";
import { useRef, useState } from "react";

export default function Slider() {
    const [disableButton,setDisableButton]=useState(false)
  const shortRef = useRef([]);
  const container = useRef();

  const descRef = useRef([]);
  const titleRef = useRef([]);
  const { curSlide, items, nextSlide, prevSlide, direction } = useSliderR3f();
  let prevIdx = direction === "next" ? curSlide - 1 : curSlide + 1;
  if (prevIdx === items.length) {
    prevIdx = 0;
  } else if (prevIdx === -1) {
    prevIdx = items.length - 1;
  }
  useGSAP(() => {
    gsap.set(
      shortRef.current,
      {
        autoAlpha: 0,
        yPercent: 100,
        zIndex:-1,
      },
      gsap.set(descRef.current,{  autoAlpha: 0,
        yPercent: 100,}),
      { scope: container },
    );

    gsap.set(shortRef.current[0], {
      autoAlpha: 1,
      yPercent: 0,
    });
    // gsap.set(descRef.current[0],{
    //     autoAlpha:1,yPercent:0
    // })
  }, []);
  useGSAP(
    () => {
      const current = shortRef.current[curSlide];
      const previous = shortRef.current[prevIdx];
      const curentDesc=descRef.current[curSlide];
      const prevDes=descRef.current[prevIdx];
      setDisableButton(true);
     const tl = gsap.timeline({onComplete:()=>{setDisableButton(false)}});

      tl.to(previous, {
        autoAlpha: 0,
        yPercent: 100,
        duration: 0.8,
      });

      tl.fromTo(
        current,
        {
          autoAlpha: 1,
          yPercent: -100,
        },
        {
          autoAlpha: 1,
          yPercent: 0,
          duration: 0.8,
          ease: "power2.out",
        },
        0,
      );
      const tl2=gsap.timeline()
       tl2.to(prevDes,{xPercent:-100,duration:0.4,ease:'bounce.in'})
      tl2.to(prevDes,{autoAlpha:0})
      tl2.fromTo(curentDesc,{autoAlpha:1,xPercent:-100,skewX:20},{autoAlpha:1,xPercent:0,duration:0.8,skewX:0})
     
    },
    {
      dependencies: [curSlide],
      scope: container,
    },
  );
  return (
    <div
      ref={container}
      className="grid place-content-center h-full select-none overflow-hidden pointer-events-none relative z-10"
    >
      {/* MIDDLE CONTAINER */}
      <div className="h-auto w-screen aspect-square max-h-[75vh] md:w-auto md:h-[75vh] md:aspect-[3/4] relative max-w-[100vw]">
        {/* TOP LEFT */}
        <div className="w-48 md:w-72 left-4 md:left-0 md:-translate-x-1/2 absolute -top-8 ">
          <h1
            className="relative antialiased overflow-hidden font-display 
                    text-[5rem] h-[4rem]  leading-[4rem]
                    md:text-[11rem] md:h-[7rem]  md:leading-[7rem] font-bold text-white block"
          >
            {items.map((_item, idx) => (
              <span
                ref={(el) => (shortRef.current[idx] = el)}
                key={idx}
                className="absolute top-0 left-0 md:text-center w-full"
              >
                {items[idx].short} 
              </span>
            ))}
          </h1>
        </div>
        {/* MIDDLE ARROWS */}
        <button 
        disabled={disableButton}
          className="absolute left-4 md:-left-14 top-1/2 -translate-y-1/2 pointer-events-auto"
          onClick={prevSlide}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth={1.5}
            stroke="currentColor"
            className="w-8 h-8 stroke-white hover:opacity-50 transition-opacity duration-300 ease-in-out"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M10.5 19.5 3 12m0 0 7.5-7.5M3 12h18"
            />
          </svg>
        </button>
        <button
        disabled={disableButton}
          className="absolute right-4 md:-right-14 top-1/2 -translate-y-1/2 pointer-events-auto"
          onClick={nextSlide}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth={1.5}
            stroke="currentColor"
            className="w-8 h-8 stroke-white hover:opacity-50 transition-opacity duration-300 ease-in-out"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3"
            />
          </svg>
        </button>

        {/* BOTTOM RIGHT */}
        <div className="absolute right-4 md:right-auto md:left-full md:-ml-20 bottom-8">
          <h2
            className="antialiased font-display font-bold 
            text-transparent text-outline-0.5 
            block overflow-hidden relative w-[50vw]
            text-5xl h-16
            md:text-8xl md:h-28"
          >
            {items.map((item, idx) => (
              <div
                key={idx}
                className="absolute top-0 left-0 w-full text-right md:text-left"
              >
                {item.title.split("").map((char, idx) => (
                  <span
                    key={idx}
                    className="inline-block" // to make the transform work (translateY)
                  >
                    {char}
                  </span>
                ))}
              </div>
            ))}
          </h2>
        </div>
        <div className="absolute right-4 md:right-auto md:left-full md:top-full md:-mt-10 bottom-8 md:bottom-auto">
          <p className="text-white w-64 h-12 overflow-hidden text-sm font-thin italic ml-4 relative">
            {items.map((item, idx) => (
              <span 
              ref={(el)=>{descRef.current[idx]=el}}
                key={idx}
                className="absolute top-0 left-0 w-full text-right md:text-left"
              >
                {item.description}
              </span>
            ))}
          </p>
        </div>
      </div>
    </div>
  );
}
